...
class ProductDescriptionSummary extends Model
{
  ...
  static function multiAccessRoutes()
  {
    Route::group(['prefix' => 'product-description-summary', 'namespace' => '\App\Modules\Admin\Models'], function () {
      ...
      Route::get('', [self::class, 'getProductDescriptionSummaries'])->name($gen(''))->defaults('ex', __e('ss,a', 'edit', true))->middleware('auth:super_admin,admin');
      Route::post('create', [self::class, 'createProductDescriptionSummary'])->name($gen('.create_product_desc'))->middleware('auth:super_admin,admin');
      Route::put('{productModel}/edit', [self::class, 'editProductDescriptionSummary'])->name($gen('.edit_product_desc'))->middleware('auth:super_admin,admin');
    });
  }
